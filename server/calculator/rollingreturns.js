...
// const fetchFund= await axios.get(`https://api.mfapi.in/mf/100219`);
// let fundData = fetchFund.data.data;

const fetchFund = {"data": [
    {
      "date": "31-10-2025",
      "nav": "157.47070"
    },
...
for(let i=0; i<fundData.length; i++){
    ...
    //const {date,month,year} = fetchDate(fund);
...
console.log(`2 Year Avg Rolling Returns = ${avgRollingCAGR}`);
console.log(`2 Year Min Rolling Returns = ${minCAGR}`);
console.log(`2 Year Max Rolling Returns = ${maxCAGR}`);

//If the year does not exist move back ==> Break the loop

/*

    //Iterate through the whole array and find the fund at same data;
    for(let i=0; i<fundData.length; i++){
        
        let found = false;

        const {newDate,newMonth,newYear} = fetchDate(fundData[i]);

        if(newYear === (year-yearsReturn) && newMonth === month){

            //Check if we can get the exact date, if exact date does not exists, take the next date;
            if(newDate === date || newDate < date ){
                found = true;
                // console.log(`Current Fund Date : ${fundDate} \nNew Fund Date : ${newFundDate}`);
                // console.log(`Current Fund Nav: ${currNav} \nNew Fund Nav : ${newNav}\n`);
            }
        }
