...
export default function RollingReturns(){
    ...
    return(
        ...
        <div className="flex flex-col gap-8">
            ...
            <div className="text-center">Rolling Returns</div>
            
            <div className="grid grid-cols-4 gap-4">

                {/* Row 0 */}
                <div className="col-span-4 text-center"> 3 year Returns </div>
                ...
                {/* Row-2 */}
                <div>Min</div>
                <div>{returns.rollingReturns["3Y"].fund.min}</div>
                <div>{returns.rollingReturns["3Y"].benchmark.min}</div>
                <div>{returns.rollingReturns["3Y"].benchmark.min}</div>
                ...
                <div>Max</div>
                <div>{returns.rollingReturns["5Y"].fund.max}</div>
                <div>{returns.rollingReturns["5Y"].benchmark.max}</div>
                <div>{returns.rollingReturns["5Y"].benchmark.max}</div>
                ...
                <div>Avg</div>
                <div>{returns.rollingReturns["10Y"].fund.avg}</div>
                <div>{returns.rollingReturns["10Y"].benchmark.avg}</div>
                <div>{returns.rollingReturns["10Y"].benchmark.avg}</div>
